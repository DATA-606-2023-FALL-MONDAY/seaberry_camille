...
class SVImageDownloader:
    def __init__(self, key_to_sec, save_dir, sleep_time=0.0):
    ...
    def download(self, rtuple):
        ...
        try:
            ...
            self.download_image(
                panoid=row["panoid"], head=head, keysec=keysec, save_path=self.save_dir
            )
        except BaseException as e:
            traceback.print_exception(*sys.exc_info())
            return {"panoid": row["panoid"], "heading": head, "exception": str(e)}
        return {"panoid": None}
...
class ParallelSVImageDownloader:
    ...
    def download(self, df, sample_frac=1.0):
        ...
        with mp.Pool(
            ...
            initargs=(self.key_to_sec, self.save_dir, self.sleep_time),
        ) as p:
            df = list(
                tqdm(p.imap(_download, df.iterrows()), total=len(df), smoothing=0.1)
            )

        image_errors = pd.DataFrame(df) # doesn't actually seem to return anything
        image_errors.dropna(subset=["panoid"], inplace=True)
        return image_errors


def download_streetview_image(key, sec):
    # editing to fit project file structure
    df = pd.read_csv("data/sv_meta.csv")
    downloader = ParallelSVImageDownloader(
        key_to_sec=[(key, sec)], save_dir="./data/streetview/images"
    )
    errs = downloader.download(df)
    # errs.to_csv('src/dataprep/streetview/sv_errors.csv', index=False)
